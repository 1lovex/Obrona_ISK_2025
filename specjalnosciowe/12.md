...
Sieć WAN (Wide Area Network - sieć rozległa) łączy urządzenia na dużych obszarach geograficznych (kraje, kontynenty). Działa głównie w warstwie fizycznej oraz łącza danych modelu OSI (Open Systems Interconnection - model odniesienia dla komunikacji między systemami otwartymi), wykorzystując infrastrukturę operatorów telekomunikacyjnych.

### 1. Protokoły warstwy łącza danych (L2 - warstwa łącza danych)

#### HDLC (High-Level Data Link Control)

Bitowy, synchroniczny protokół, który stanowił fundament dla rozwoju nowoczesnej transmisji danych.

**Jak o nim opowiedzieć:** To protokół "porządkowy". Zarządza tym, jak bity są pakowane w ramki, synchronizuje zegary obu urządzeń i pilnuje, by dane nie zaginęły dzięki sumie kontrolnej FCS (Frame Check Sequence - sekwencja sprawdzająca ramkę).
...
**Jak o nim opowiedzieć:** To "policjant" protokołu X.25. Jego jedynym zadaniem jest dopilnowanie, by połączenie między użytkownikiem (DTE - Data Terminal Equipment - urządzenie końcowe użytkownika) a operatorem (DCE - Data Circuit-terminating Equipment - urządzenie operatora) było bezbłędne. Jeśli ramka zginie, LAP-B natychmiast wymusza jej retransmisję.
...
**Klucz:** Wykorzystuje identyfikatory DLCI (Data Link Connection Identifier - identyfikator połączenia łącza danych) do kierowania ruchem wewnątrz "chmury" operatora.

#### ATM (Asynchronous Transfer Mode - tryb transmisji asynchronicznej)

Technologia oparta na bardzo małych, stałych jednostkach danych.

**Jak o nim opowiedzieć:** Kluczem jest komórka 53 bajty. Stały rozmiar pozwala urządzeniom przełączać dane sprzętowo, z ogromną prędkością (622 Mb/s+). Jest idealny do transmisji głosu i wideo, bo dzięki małym komórkom nie powstają "korki" w transmisji (brak opóźnień jitter - fluktuacja opóźnienia).

### 2. Protokoły warstwy sieciowej i szkieletowej (L3 - warstwa sieciowa / L2.5)
...
**Jak o nim opowiedzieć:** Zamiast analizować skomplikowane adresy IP na każdym routerze, MPLS nakleja na pakiety proste "etykiety". Routery (LSR - Label Switching Router - router przełączający etykiety) czytają tylko te etykiety (jak kody kreskowe na paczkach), co drastycznie przyspiesza przesył danych w dużych sieciach operatorskich.
